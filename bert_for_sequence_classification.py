...
def train(model, optimizer, scheduler, train_dataloader, epoch, device, args):
    ...
    with torch.profiler.profile(
      ...
      for step, batch in enumerate(train_dataloader):
        ...
        # The call to `model` always returns a a dictionary, so we need to pull the
        # loss value out of the dictionary.
        loss = outputs['loss']
        ...
        if args.view_graphs:
            import torchviz
            pytorch_backward_graph = torchviz.make_dot(outputs['loss'], params=dict(list(model.named_parameters())))
